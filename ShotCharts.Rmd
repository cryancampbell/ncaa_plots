...
for (q in c(2,8,5,12,3,13,9,1,4,11,6,7,10,14)) {
  ...
  bestScorer$logo[qCount] <- as.character(teamLogo$logo[which(teamLogo$teamName == bestScorer$teamName[qCount])])
  
#  bestScorer$shooterEfficiency[qCount] <- bestScorer$points[qCount] / bestScorer$shots[qCount] - 
#            FGType * bestScorer$quadFGPer[qCount]
  bestScorer$shooterEfficiency[qCount] <- 100 * ( ( bestScorer$points[qCount] / FGtype ) / 
...
#
#make a shot trans with only the best scorers' shot locations
#   do this with subset, shooter & zone | shooter & zone | ...

shotsScorer <- subset(shotsTrans, shooter == bestScorer$shooter[1] & shotQuadrant == rownames(bestScorer)[1] |
                   shooter == bestScorer$shooter[2] & shotQuadrant == rownames(bestScorer)[2] |
                   shooter == bestScorer$shooter[3] & shotQuadrant == rownames(bestScorer)[3] |
                   shooter == bestScorer$shooter[4] & shotQuadrant == rownames(bestScorer)[4] |                     
                   shooter == bestScorer$shooter[5] & shotQuadrant == rownames(bestScorer)[5] |
                   shooter == bestScorer$shooter[6] & shotQuadrant == rownames(bestScorer)[6] |
                   shooter == bestScorer$shooter[7] & shotQuadrant == rownames(bestScorer)[7] |
                   shooter == bestScorer$shooter[8] & shotQuadrant == rownames(bestScorer)[8] |
                   shooter == bestScorer$shooter[9] & shotQuadrant == rownames(bestScorer)[9] |                     
                   shooter == bestScorer$shooter[10] & shotQuadrant == rownames(bestScorer)[10] |
                   shooter == bestScorer$shooter[11] & shotQuadrant == rownames(bestScorer)[11] |
                   shooter == bestScorer$shooter[12] & shotQuadrant == rownames(bestScorer)[12] |
                   shooter == bestScorer$shooter[13] & shotQuadrant == rownames(bestScorer)[13] |
                   shooter == bestScorer$shooter[14] & shotQuadrant == rownames(bestScorer)[14])

shotsScorer$zoneEff <- 0

for (q in 1:14) {
  shotsScorer$zoneEff <- ifelse(shotsScorer$shooter == bestScorer$shooter[q], 
                              bestScorer$shooterEfficiency[q], shotsScorer$zoneEff)
  }
...
#tweak name locations
yPos <- bestScorer$yPlot
xPos <- bestScorer$xPlot
...
bestScorer$yPlot <- yPos
bestScorer$xPlot <- xPos
...
ggplot() + 
  ...
  scale_y_continuous(breaks = c(0, 23.5, 47)) +
  scale_x_continuous(breaks = c(0, 12.5, 25, 37.5, 50)) +
  geom_point(data = shotsScorer, 
             aes(x = baseline, y = depth, col = zoneEff), size = 2.4, pch = 15, alpha = .5) +
  geom_text(data = bestScorer, 
            aes(x = xPlot, y = yPlot + 1, label = shooter), alpha = 1, size = 4) +
  geom_image(data = bestScorer, 
             aes(x = xPlot, y = yPlot + 3, image = logo), size = .05, alpha= 1) +
  #scale_color_gradient2(midpoint=mean(bestScorer$shooterEfficiency), low="#560DFF", 
  #                      mid="#D800FF", high="#FF0D50") +
  scale_color_gradientn(colours = terrain.colors(12)[10:1]) +
  #scale_color_gradientn(colors = colorInterpolate(1:6,1:6)) +
  xlab("test") + ylab("") + ggtitle("Leading College Scorer by Zone", subtitle = "(High-Major, Min 3 games charted, 20 shots taken)") +
  theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.ticks.x = element_blank(),
        axis.ticks.y = element_blank(), axis.title = element_blank(), panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), text = element_text(size = 12, family = "Arial"), 
        plot.title = element_text(size=20), plot.subtitle = element_text(size=10))

ggsave("plots/bestScorer_final.png", height = 7, width = 7)
