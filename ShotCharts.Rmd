...
bestScorer <- ggplot(reducedShots, aes(baseline, depth)) + 
  ylim(0,40)  + xlim(0,50) + 
...
for (q in c(2,8,5,12,3,13,9,1,4,11,6,7,10,14)) {
  ...
  print(c(colnames(dataShooters)[q],
          ...
          xPlot,
          yPlot))
  bestScorer <- bestScorer + geom_text(x = xPlot, y = yPlot, label = player, alpha = .02, color = "blue", angle = 20)
  bestScorer <- bestScorer + geom_text(x = xPlot, y = yPlot + 2, label = ppg, alpha = .01, color = "black", angle = 20)
}
